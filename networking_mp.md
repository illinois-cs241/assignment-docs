...
**Note**: Epoll may return an event on a closed file descriptor. To avoid this, explicitly call epoll_ctl(EPOLL_CTL_DEL) before calling close().
...
Part 1 requires **full** client functionality.
...
Part 3 is full server-client functionality. All tests will run, including the stress test.
